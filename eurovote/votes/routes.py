...
def cast():
    ...
    if form.validate_on_submit():
        cvote = Vote(name='Angola',song=form.song.data, outfit=form.outfit.data, performance=form.performance.data,user_id=current_user.id)
        db.session.add(cvote)
        db.session.commit()
        flash('Your vote has been cast.','success')
        return redirect(url_for('vote.cast'))
